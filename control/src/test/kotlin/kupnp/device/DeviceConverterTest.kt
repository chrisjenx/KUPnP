...
class DeviceConverterTest {
    ...
    val xmlBuffer = Okio.buffer(Okio.source(File(resourceDir, "gateway.xml").toPath())).readUtf8()

    @Test
    fun parseGatewayDeviceDescriptionBody() {
        server.enqueue(MockResponse().setBody(xmlBuffer))

        val result = service.getRootDeviceDescription("/").toBlocking().single()
        val expected = DeviceRoot(
                SpecVersion(1, 0),
                Device(
